...
public class RUHungry {
    ...
    for (int i = 0; i < numCategories; i++) {
        ...
        for (int j = 0; j < numDishes; j++) {
            StdIn.readLine();
            String dishName = StdIn.readLine(); // Read the dish name
    ...
    int ingredientID = newNode.getIngredient().getID(); //retrieve the ingredientID from the new node
    int i = ingredientID % stockVarSize; // //Calculate the has index using the hash function 
    StockNode newStockNode = new StockNode(newNode.getIngredient(), null); //create a new node with the given newNode
    if (stockVar[i]==null){ //if LL at the calculated index is empty, assign the new node
        stockVar[i] = newStockNode;
    } else { //if the linked list is not empty, insert the new node at the front
        newStockNode.setNextStockNode(stockVar[i]);
        stockVar[i]=newStockNode;
    }
    }
    ...
    // If ingredientName is null, search using ingredientID
    if (ingredientName == null) {
        StockNode stockNode = findStockNode(ingredientID);
        // If ingredient with given ID is found, update stock amount
        if (stockNode != null) {
            Ingredient ingredient = stockNode.getIngredient();
            int currentStockAmount = ingredient.getStockLevel();
            // Update stock amount by adding stockAmountToAdd
            ingredient.setStockLevel(currentStockAmount + stockAmountToAdd);
        }
    }
    // If ingredientID is -1, search using ingredientName
    else if (ingredientID == -1) {
        // Iterate over stockVar to find the ingredient with the given name
        for (int i = 0; i < stockVar.length; i++) {
            StockNode currentStockNode = stockVar[i];
            while (currentStockNode != null) {
                Ingredient ingredient = currentStockNode.getIngredient();
                if (ingredient.getName().equals(ingredientName)) {
                    // Ingredient found, update stock amount
                    int currentStockAmount = ingredient.getStockLevel();
                    // Update stock amount by adding stockAmountToAdd
                    ingredient.setStockLevel(currentStockAmount + stockAmountToAdd);
                    // Exit the method as ingredient is found and updated
                    return;
                }
    ...
    public void updatePriceAndProfit() {
     // Iterate over the entire menuVar
     for (int i = 0; i < menuVar.length; i++) {
        MenuNode currentNode = menuVar[i];
        while (currentNode != null) {
            // Get the dish from the current menu node
            Dish dish = currentNode.getDish();
            // Initialize dish cost
            double dishCost = 0.0;
            // Get the ingredient IDs required for the dish
            int[] ingredientIDs = dish.getStockID();
            // Compute dish cost by summing up the cost of each ingredient
            for (int j = 0; j < ingredientIDs.length; j++) {
                // Find the stock node corresponding to the ingredient ID
                StockNode stockNode = findStockNode(ingredientIDs[j]);
                // If stock node is found, add its cost to the dish cost
                if (stockNode != null) {
                    Ingredient ingredient = stockNode.getIngredient();
                    dishCost += ingredient.getCost();
                }
            }
            // Compute dish price (assuming 20% markup)
            double dishPrice = dishCost * 1.2;
            // Update the price of the dish
            dish.setPriceOfDish(dishPrice);
            // Update the profit of the dish
            double profit = dishPrice - dishCost;
            dish.setProfit(profit);
            // Move to the next menu node
    ...
    // If the transactionVar is null, meaning the list is empty, assign the new node as the first node
    if (transactionVar == null) {
        transactionVar = newNode;
        return; // Exit the method since the list was empty
    }
    ...
    public void order (String dishName, int quantity){
        MenuNode currentNode = findDish(dishName);
        // Check if the dish is available
        if (checkDishAvailability(dishName, quantity)) {
            // Attempt to sell the dish
            double dishProfit = currentNode.getDish().getProfit() * quantity;
            TransactionData transactionData = new TransactionData("order", dishName, quantity, dishProfit, true);
            ...
            return; // Exit the method after successful sale
        } else {
            // Create a failed transaction for the original dish
            TransactionData failedTransaction = new TransactionData("order", dishName, quantity, 0, false);
            addTransactionNode(failedTransaction);
    
            // Get the dish node to avoid attempting the order twice
            MenuNode nextDishNode = currentNode.getNextMenuNode();
    
            // Try orders for dishes directly after the original order dish in the linked list
            while (nextDishNode != null) {
                if (checkDishAvailability(nextDishNode.getDish().getName(), quantity)) {
                    // Attempt to sell the next dish
                    double nextDishProfit = nextDishNode.getDish().getProfit() * quantity;
                    TransactionData nextDishTransaction = new TransactionData("order", nextDishNode.getDish().getName(), quantity, nextDishProfit, true);
                    addTransactionNode(nextDishTransaction);
    
                    // Update stock for the next dish
                    Dish nextDish = nextDishNode.getDish();
                    for (int ingredientID : nextDish.getStockID()) {
                        updateStock(null, ingredientID, -quantity);
                    }
                    return; // Exit the method after successful sale
                } else {
                    // Create a failed transaction for the next dish
                    TransactionData nextDishFailedTransaction = new TransactionData("order", nextDishNode.getDish().getName(), quantity, 0, false);
                    addTransactionNode(nextDishFailedTransaction);
                }
    
                // Move to the next dish in the same category
                nextDishNode = nextDishNode.getNextMenuNode();
            }
    
            // If the next dish is null, loop back to the beginning of the category and continue checking until reaching the original dish
            nextDishNode = menuVar[findCategoryIndex(currentNode.getDish().getCategory())];
            while (nextDishNode != currentNode) {
                if (checkDishAvailability(nextDishNode.getDish().getName(), quantity)) {
                    // Attempt to sell the next dish
                    double nextDishProfit = nextDishNode.getDish().getProfit() * quantity;
                    TransactionData nextDishTransaction = new TransactionData("order", nextDishNode.getDish().getName(), quantity, nextDishProfit, true);
    ...
    public double profit () {   
        ...
        while (currentTransaction != null) {
            // Check if the transaction is for an order or a restock and if it was successful
            if ((currentTransaction.getData().getType().equalsIgnoreCase("order") || currentTransaction.getData().getType().equalsIgnoreCase("restock")||currentTransaction.getData().getType().equalsIgnoreCase("donation")) && currentTransaction.getData().getSuccess()) {
                // Add the profit of the successful order or restock to the total profit
    ...
    // Check if profit for the day is greater than $50
    if (profit() > 50) {
        // Check if there is enough stock for the donation
        if (checkStockAvailability(ingredientName, quantity)) {
            // Update stock for the donation
    ...
    private boolean checkStockAvailability(String ingredientName, int quantity) {
        ...
        return false; // Return false if the ingredient is not found in stock
    }
    ...
    double restockCost = calculateRestockCost(ingredientName, quantity);
    
    // Check if there's enough profit to cover the restock cost
    if (profit() >= restockCost) {
        // Update the stockroom accordingly
        updateStock(ingredientName, -1, quantity);
        
        // Record the transaction
        TransactionData transactionData = new TransactionData("restock", ingredientName, quantity, -restockCost, true);
        addTransactionNode(transactionData);
    } else {
        // Insufficient profit to restock, record failed transaction
        TransactionData transactionData = new TransactionData("restock", ingredientName, quantity, 0, false);
        addTransactionNode(transactionData);
    }
    }
    private double calculateRestockCost(String ingredientName, int quantity) {
        // Find the cost of the ingredient
        StockNode stockNode = findStockNode(ingredientName);
        if (stockNode != null) {
            double costPerUnit = stockNode.getIngredient().getCost();
            // Calculate the total cost of restocking
            return costPerUnit * quantity;
        }
        return 0; // Ingredient not found, return 0 cost
    }
